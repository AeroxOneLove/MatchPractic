...
class MatchService:
    ...
    def extract_skills_from_text(self, text: str) -> List[str]:
        ...
        for keyword in skill_keywords:
            matches = re.finditer(fr"{keyword}\s+([\w\s/]+)", text, re.IGNORECASE)
    ...
    Не добавляй никаких комментариев!"""
        ...
        try:
            response = ollama.chat(model="mistral:latest", messages=[{"role": "user", "content": PROMPT.format(text=text)}])
    ...
    def compare_skills(self, resume_skills: List[str], job_skills: List[str]) -> Dict:
        ...
        skill_embeddings = {skill: self.get_embedding_ollama(self.preprocess_text(skill)) for skill in set(resume_skills + job_skills)}
    ...
    def calculate_match(self, resume: Dict, job: Dict) -> Dict:
        ...
        if not experience_match:
            ...
            result['didnt_match'].append(f"Опыт работы: {resume['work_experience']} лет (требуется {job['work_experience']}+, не хватает {exp_diff:.1f} лет)")
        experience_score = 20 if experience_match else max(0, 20 * (1 - (job['work_experience'] - resume['work_experience']) / job['work_experience']))
        ...
        skills_score = 50 * (skills_result['matched_skills'] / skills_result['total_skills']) if skills_result['total_skills'] > 0 else 0
